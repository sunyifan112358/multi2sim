...
void Binary::ReadNotes(BinaryDictEntry *dict_entry)
{
	...
	elf::ProgramHeader32 *ph = dict_entry->pt_note_segment;
...
void Binary::ReadSegments()
{
	...
	for (unsigned i = 0; i < dict.size(); i++)
	{
		...
		for (int j = 0; j < getNumProgramHeaders(); j++)
		{
			...
			elf::ProgramHeader32 *ph = getProgramHeader(j);
...
void Binary::ReadSections()
{
	...
	{
		// Get encoding dictionary entry
		elf::ProgramHeader32 *load_segment = dict_entry->pt_load_segment;
		elf::ProgramHeader32 *note_segment = dict_entry->pt_note_segment;
		assert(load_segment);
		assert(note_segment);
		...
		for (int i = 0; i < getNumSections(); i++)
		{
			...
			elf::Section32 *section = getSection(i);
