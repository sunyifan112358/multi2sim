...
#ifndef SRC_LIB_ELF_FILE32_H
#define SRC_LIB_ELF_FILE32_H
...
/// A File32 file is an 32-bit elf file
class File32: public File
{
	...
	Section32 *string_table;

	// List of sections
	std::vector<std::unique_ptr<Section32>> sections;

	// List of program headers
	std::vector<std::unique_ptr<ProgramHeader32>> program_headers;

	// List of symbols
	std::vector<std::unique_ptr<Symbol32>> symbols;
...
public:
	...
	File32(const std::string &path, bool read_content = true);
	...
	///	file. If false, read only the ELF reader. The ELF reader will
	///	return no program header, section, or symbol for the file.
	File32(const char *buffer, unsigned size, bool read_content = true);
	...
	Section32 *getSection(int index) const
	{
	...
	Section32 *getSection(const std::string &name) const;
	...
	/// \endcode
	const std::vector<std::unique_ptr<Section32>> &getSections() const
	{
		return sections;
	}
	...
	ProgramHeader32 *getProgramHeader(int index) const
	{
	...
	const std::vector<std::unique_ptr<ProgramHeader32>>
	...
	/// value given in \a index is out of range.
	Symbol32 *getSymbol(int index) const
	{
	...
	/// log(n).
	Symbol32 *getSymbol(const std::string &name) const;

	/// Return a constant reference to the list of symbols for convenient
	/// iteration.
	const std::vector<std::unique_ptr<Symbol32>> &getSymbols() const
	{
		return symbols;
	}
	...
	Symbol32 *getSymbolByAddress(unsigned int address) const;
