...
ProgramHeader32::ProgramHeader32(File32 *file, int index, unsigned info_offset) :
		ProgramHeader(index),
		file(file)
{
	// Initialize
	...
	if (info_offset + sizeof(Elf32_Phdr) > file->getSize())
...
void ProgramHeader32::getStream(std::istringstream &stream, unsigned int offset,
