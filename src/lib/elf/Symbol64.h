...
#ifndef SRC_LIB_ELF_SYMBOL64_H
#define SRC_LIB_ELF_SYMBOL64_H
...
public:
	...
	Symbol64(File64 *file, Section64 *section, unsigned info_offset);
	...
	/// called.
	static bool Compare(const std::unique_ptr<Symbol64> &a,
			const std::unique_ptr<Symbol64> &b);

	/// Return the section associated with the symbol
	Section64 *getSection() const { return section; }
	...
	/// getters instead.
	Elf64_Sym *getRawInfo() const { return info; }

	/// Return field \a st_value
	Elf64_Addr getValue() const { return info->st_value; }

	/// Return field \a st_size
	Elf64_Word getSize() const { return info->st_size; }
	...
	Elf64_Section getShndx() const { return info->st_shndx; }
